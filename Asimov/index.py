...
option_month = [{'label': 'Ano todo', 'value': 0}]
for i, j in zip(df_cru['Mês'].unique(), df['Mês'].unique()):
    option_month.append({'label': i, 'value': j})
option_month = sorted(option_month, key=lambda x: x['value'])
...
def graph5(month, toggle):
    ...
    fig5.add_trace(go.Indicator(
        mode ='number+delta',
        title = {"text": f"<span>{df_5['Consultor'].iloc[0]} - Top Consultant</span><br><span style='font-size:70%'>Em vendas - em relação a média</span>"},
    ...
    fig6.add_trace(go.Indicator(
        mode ='number+delta',
        title = {"text": f"<span>{df_6['Equipe'].iloc[0]} - Top Team</span><br><span style='font-size:70%'>Em vendas - em relação a média</span>"},
...
def graph9(month, team, toggle):
    ...
    fig9.update_layout(main_config, height=300, template=template, showlegend=False)
    return fig9
...
def graph10(team, toggle):
    ...
    fig10.update_layout(main_config, height=300, template=template, showlegend=False)
    return fig10
